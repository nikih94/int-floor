...
public class FeatureExtraction {
    ...
    //vrne podatek na 25 vrstici bufferja tako izris je vsklajen s featurji
    public float[] getPetindvajsetaVrstica(){
    ...
    //pregledaj odvod vrstice 24 in 25 sliding windowa in ce sum odvodov vseh vrstic je vecji od meje sprozi registriranje
    public boolean startRecording(){

        //pregledaj ali je sliding window polhen?
        if(window[0].size() < 40){
            return false;
        }
        ...
        for(int i = 0; i < window.length ; i++ ){
            vsota += Math.abs(window[i].get(30) - window[i].get(31));
        }
        if( vsota > meja ){
    ...
    //izbrise vse elemente iz arraylista
    public void pocisti(){
        for(int i = 0; i < window.length ; i++ ){
            window[i].clear();
        }
