...
#zbrise vse 0 
#resi samo vrstice, ki imajo vec kot 1 ali 2 ali 3 meritve vecj od 0
#zelimo ohraniti vrstni red meritev
...
for row in df.itertuples():
    ...
    for r in m:
        for v in r:
            if v > 0:
                i=i+1
        if i >= 1: #poskusi postavit na 2 ali 3   - oznaci stevilo vrednost vecjih od 0 v polju
            newDF.loc[len(newDF.index)]=m.ravel() ##vstavi element na koncu


index_orig = newDF.index
for k in range(3):
    for i in index_orig:
        row = newDF.iloc[i,:]
...
#resi na datoteko
newDF.to_csv('/home/niki/Desktop/tesi/data_registered/testnipodatkiTRUE/processedCNN/10padcev.txt', index = False)
